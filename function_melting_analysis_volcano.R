...
# EEV1401 mut c(77.5,79.75)
# EEV1401 wt c(79.75,82)

peak.delimitation = function(data, trange.mut, trange.wt){
  ...
  if(!is.na(peaks$phase.genotype[mainpeak]) & nrow(peaks)>1){
    ...
    if(nrow(peaks)==2){
      ...
      if(peaks$phase.genotype[mainpeak]=="wt"){
        ...
        peaks$phase.genotype[which(c(1,2)!=mainpeak)] = ifelse(which(c(1,2)!=mainpeak)==1, 'mut', NA) # should look at melting range to be sure differenciate het and mut
...
HRM = function(rawd, ampli=NULL, trange.mut, trange.wt){
  ...
  normd = lapply(split(rawd, rawd$id), function(x){
    ...
    x = x[order(x$temp),]
    p = try(peak.delimitation(x, trange.mut = trange.mut, trange.wt = trange.wt))
